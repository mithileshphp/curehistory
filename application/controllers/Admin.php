...
class Admin extends CI_Controller {
    ...
    public function get_agent()
   {
    ...
    public function Add_agent(){
        if($this->session->userdata('user_login_access') != False) { 
        $this->load->view('backend/add-agent');
        }
    else{
		redirect(base_url() , 'refresh');
	}            
    ...
    public function Add_doctor(){
        if($this->session->userdata('user_login_access') != False) { 
        $this->load->view('backend/add-doctor');
        }
    else{
		redirect(base_url() , 'refresh');
	}            
    ...
    $emrand = $this->input->post('name');
    $id = $this->input->post('em_id');
    $digits = 3;
    $em_id= rand(pow(10, $digits-1), pow(10, $digits)-1); 
	...
	$confirm = $this->input->post('confirm');	
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $config = array(
                ...
                'upload_path' => "./assets/images/agent",
    ...
    public function personinfo(){ 
        if($this->session->userdata('user_login_access') != False) {  
            ...
            $address = $this->input->post('address');    
            $high_risk_factors = $this->input->post('high_risk_factors');    
            $diseases_suffering_from = $this->input->post('diseases_suffering_from');
            $contact = $this->input->post('contact');
            ...
            $created_at = date('Y-m-d H:i:s');
                ...
                if (!$this->upload->do_upload('image_url')) {
                    ...
                    $data = array(
                        ...
                        'diseases_suffering_from' => $diseases_suffering_from,
                        'contact' => $contact,
                ...
                $success = $this->db->insert('patient',$data);
                #$this->confirm_mail_send($email,$pass_hash);        
                #$this->session->set_flashdata('feedback','Successfully Created');
                echo "fdgd";                       
                }
    ...
    public function dsff (){
        ...
        $success = $this->employee_model->ongoing_medical($data);
        #$this->confirm_mail_send($email,$pass_hash);        
        #$this->session->set_flashdata('feedback','Successfully Created');
        echo "Successfully Added";   
    ...
    public function ongoing_medical(){
        $name = $this->input->post('name');    
        $guantity = $this->input->post('guantity');    
        $frequency = $this->input->post('frequency');    
        $duration = $this->input->post('duration');
        $relation = $this->input->post('relation');
        $daytimings = $this->input->post('daytimings');
        $data = array(
            'name' => $name,
            'guantity' => $guantity,
            'frequency' => $frequency,
    ...
    public function update_blood_report(){
        $testname = $this->input->post('testname');    
        $value = $this->input->post('value');    
        $range_value = $this->input->post('range_value');    
        $data = array(
            'testname' => $testname,
            'value' => $value,
            'range_value' => $range_value,
        );
        $success = $this->employee_model->update_blood_report($data);
        #$this->confirm_mail_send($email,$pass_hash);        
        #$this->session->set_flashdata('feedback','Successfully Created');
        echo "Successfully Added";   
    ...
    public function patient()
	{
        ...
        //   $data= array();
        // redirect('employee/Employees');
     $this->load->view('backend/patient');
       
	}

    public function Add_patient(){
        if($this->session->userdata('user_login_access') != False) { 
        $this->load->view('backend/add-patient');
        }
    else{
		redirect(base_url() , 'refresh');
	}            
    ...
    public function qrcode()
	{
        ...
        //   $data= array();
        // redirect('employee/Employees');
     $this->load->view('backend/qrcode');
       
	}

    public function Add_qrcode(){
        if($this->session->userdata('user_login_access') != False) { 
        $this->load->view('backend/add-qrcode');
        }
    else{
		redirect(base_url() , 'refresh');
	}            
